...
show_setup_checklist() {
    ...
    echo ""
    echo "🔐 FOR WEB APPS:"
    echo "  [ ] Log into all apps BEFORE recording"
    echo "  [ ] Don't record login screens (shows email/password)"
    echo "  [ ] Start recording once you're already logged in"
    echo "  [ ] Make sure demo data/content is ready"
    echo ""
...
record_app() {
    ...
    echo "  1. Open your app and LOG IN first (don't record login!)"
    echo "  2. Navigate to the main dashboard/view you want to show"
    echo "  3. Press Cmd+Shift+5 to open Screen Recording"
    echo "  4. Select 'Record Selected Window' or 'Record Entire Screen'"
    echo "  5. Click 'Record' button"
    echo "  6. Perform the actions described above"
    echo "  7. Click Stop button in menu bar when done"
    echo ""
    echo "⚠️  IMPORTANT: Start recording AFTER you're logged in!"
    echo ""
